...
os.environ['TORCH_HOME']='/mnt/data1/cc/DE'
...
oc_dataset = OC_Dataset(
    root='./Colon_data/RealData',
    depth_transform=depth_transform)
oc_dataloader = torch.utils.data.DataLoader(dataset=oc_dataset, batch_size=oc_batch, shuffle=True, num_workers=1,
                                            drop_last=True)
...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for i, data in enumerate(dataset, start=epoch_iter):
        ...
        ############## Forward Pass ######################
        # VC forward
        losses, generated = model(Variable(data['label']), Variable(data['inst']),Variable(data['image']), Variable(data['feat']), infer=save_fake, is_OC=False)

        # if generated is None:
        #     print("Error: generated is None")
        # else:
        #
        # apply mask to ignore pure black pixels
        # mask = (Variable(data['image']) != 0).float()
        # mask = mask.expand_as(generated)
        #
        # masked_losses = []
        # mask_sum = mask.sum()
        # for loss in losses:
        #     masked_loss = (loss * mask).sum() / mask_sum  # average of non-zero pixels
        ...
        if epoch > 70:
            ...
            for oc_index in range(oc_batch):
                generated_oc_depth = model(
                    label=torch.stack([source_for_depth[oc_index], target_for_depth[oc_index]], dim=0), inst=None,
                    image=None, feat=None, infer=save_fake, is_OC=True)
                oc_warp_loss += warp_loss(source_for_op[oc_index], target_for_op[oc_index],
                                          generated_oc_depth[0].unsqueeze(0), generated_oc_depth[1].unsqueeze(0))
    ...
    # plt.title('mean_relative_l1_error')
    #
    # plt.subplot(1, 3, 2)
    # plt.plot(finetune_epoch, mle)
    # plt.title('mean_l1_error')
    #
    # plt.subplot(1, 3, 3)
    # plt.plot(finetune_epoch, rmse)
    # plt.title('r_mean_square_error')
    #
